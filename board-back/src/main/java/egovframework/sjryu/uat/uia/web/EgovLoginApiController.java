package egovframework.sjryu.uat.uia.web;
...
import egovframework.sjryu.uat.uia.service.EgovLoginService;
...
public class EgovLoginApiController {
	...
	public HashMap<String, Object> actionLogin(@RequestBody LoginVO loginVO, HttpServletRequest request)
			throws Exception {
	...
	public HashMap<String, Object> actionLoginJWT(@RequestBody LoginVO loginVO, HttpServletRequest request,
		...
		if (loginResultVO != null && loginResultVO.getId() != null && !loginResultVO.getId().equals("")) {
			...
			// EgovUserDetailsHelper.isAuthenticated() 가 그 역할 수행. DB에 정보가 없으면 403을 돌려 줌.
			// 로그인으로 튕기는 건 프론트 쪽에서 처리
			request.getSession().setAttribute("LoginVO", loginResultVO);
	 ...
	 * @exception Exception
	 */
	@Operation(summary = "로그아웃", description = "로그아웃 처리(JWT,일반 관계 없이)", security = {
			@SecurityRequirement(name = "Authorization") }, tags = { "EgovLoginApiController" })
